  ...
  return (
    <div className={classes.root}>
      <List component="nav" aria-label="main mailbox folders">
        {groceries.filter(item => item.section === "Frozen" && item.isPurchased === false).map(({_id, item, isPurchased}, i) => (
          <ListItem 
            button
            key={i}
      ...
      <List component="nav" aria-label="secondary mailbox folders">
        {groceries.filter(item => item.section === "Frozen" && item.isPurchased === true).map(({_id, item, isPurchased}, i) => (
          <ListItem 
            button
            key={i}
            onClick={event => updateItem(event, _id)}
